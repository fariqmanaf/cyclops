...
import { getTopicsByUser, getMahasiswaByTopic, getAttendanceDetails } from "@/service/absensi";
  ...
  const {
    ...
    const {
      ...
      queryFn: () => {
        ...
        return selectedTopic ? getMahasiswaByTopic(selectedTopic.nama) : Promise.resolve([]);
      },
      ...
      <div className="w-full max-w-6xl mx-auto p-6 space-y-4">
        ...
        <Card className="border shadow-sm">
          <CardContent className="p-8 px-2 relative">
            <div className="flex space-x-2 justify-center px-24" ref={scrollContainerRef}>
              {topics.map((topic) => (
                <button
                  key={topic.id}
                  onClick={() => handleTopicClick(topic)}
                  className={`px-4 py-6 rounded-xl font-bold transition-colors ${
                    topic.active ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-400"
                  }`}
                >
                  {topic.nama}
                </button>
              ...
              <Card key={student.id} className="border shadow-sm">
                <CardContent className="p-4">
                  <div className="flex justify-between items-center">
                    <div className="cursor-pointer" onClick={() => handleStudentClick(student)}>
                      <h3 className="font-semibold text-lg">{student.nama}</h3>
                      <p className="text-gray-500 text-sm">{student.nim}</p>
                    </div>
                    <div className="flex space-x-3 items-center">
                      <AttendanceStatus label="Alfa" color="bg-red-500" count={student.alfa} />
                      <AttendanceStatus label="Telat" count={student.telat} color="bg-yellow-500" />
                      <AttendanceStatus label="Hadir" count={student.hadir} color="bg-blue-500" />
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              {attendanceDetails.map((record) => {
                ...
                return(
                  <div key={record.logbookId} className="flex items-center justify-between border-b pb-2">
                    <p className="font-medium">{date.toLocaleDateString("id-ID", {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                    })}</p>
